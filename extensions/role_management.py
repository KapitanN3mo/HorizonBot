...
class RMS(commands.Cog):
    ...
    def gen_block_embed(cls, block: database.RoleBlock):
        ...
        for role in sorted([role for role in database.Role.select().where(database.Role.linked_block == block)],
            ...
            emb.set_footer(text=f'Блок #{block.block_id}')
...
class RoleCommands(commands.Cog):
    ...
    async def disconnect_role(self, inter: disnake.CommandInteraction, role: disnake.Role):
        ...
        if res:
            await inter.send(f'{emojis.white_check_mark} `Роль удалена`',ephemeral=True)
        else:
    ...
    async def remove_block(self, inter: disnake.CommandInteraction, block_id: int):
        ...
        if result:
            await inter.send(f'{emojis.white_check_mark}`Блок #{block_id} успешно удалён`',ephemeral=True)
        else:
    ...
    async def connect_role(self, inter: disnake.CommandInteraction, role: disnake.Role, emoji: str, role_block_id: int):
        ...
        if result:
            await inter.send(f'{emojis.white_check_mark} `Роль добавлена!`',ephemeral=True)
        else:
    ...
    async def hide_block(self, inter: disnake.CommandInteraction, block_id: int):
        ...
        try:
            ...
            return
        await mes.delete()
        await inter.send(f'{emojis.white_check_mark}`Блок {role_block.name}#{role_block.block_id} скрыт!`',ephemeral=True)
    ...
    async def show_block(self, inter: disnake.CommandInteraction, block_id: int, channel: disnake.TextChannel):
        ...
        if res:
            await inter.send(f'{emojis.white_check_mark}`Выполнено!`',ephemeral=True)
        else:
...
class BlockCreteModal(disnake.ui.Modal):
    ...
    async def callback(self, interaction: disnake.ModalInteraction, /) -> None:
        ...
        await interaction.send(f'{emojis.white_check_mark}`Блок {name} создан!`',ephemeral=True)
